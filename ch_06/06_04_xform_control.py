# 02_04_xfrom_control.py
# GOAL: Wire the FBModel xform properties to the widgets.
...
from pyfbsdk import FBVector3d, FBModelList, FBGetSelectedModels, FBSystem, FBModel
import pythonidelib
...
class XformControl(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        main_layout.addLayout(self._init_check_boxes())
    ...
    def _init_check_boxes(self)->QHBoxLayout:
        """
        Initialize the checkboxes widgets used to display the xform controls.
        """
        enable_translate = QCheckBox('Translation', self)
        enable_translate.setChecked(False)
        enable_translate.setObjectName('translation')

        enable_rotation = QCheckBox('Rotation', self)
        enable_rotation.setChecked(False)
        enable_rotation.setObjectName('rotation')

        enable_scale = QCheckBox('Scale', self)
        enable_scale.setChecked(False)
        enable_scale.setObjectName('scale')

        # Connections
        enable_translate.stateChanged.connect(self.enable_controls)
        enable_rotation.stateChanged.connect(self.enable_controls)
        enable_scale.stateChanged.connect(self.enable_controls)

        # Layout
        layout = QHBoxLayout(self)
        layout.addStretch(1)
        layout.addWidget(enable_translate)
        layout.addWidget(enable_rotation)
        layout.addWidget(enable_scale)
        layout.addStretch(1)
    ...
    def _init_slider_grp(self, grp_label:str, grp_items_labels:list=['X', 'Y', 'Z']):
        """
        ...
        self._control_group_box.setVisible(False)
    ...
    def enable_controls(self):
        """
        Shows / Hide the xform groups within the widget.
        """
        sender = self.sender()
        if sender.objectName() == 'translation':
            control_grp = self.findChild(QGroupBox, 'Translate')
        elif sender.objectName() == 'rotation':
            control_grp = self.findChild(QGroupBox, 'Rotate')
        else:
            control_grp = self.findChild(QGroupBox, 'Scaling')

        if sender.isChecked():
            control_grp.setEnabled(True)
            control_grp.setVisible(True)
            self.adjustSize()
        else:
            control_grp.setEnabled(False)
            control_grp.setVisible(False)
            self.adjustSize()
